...
static void DirectoryList(const string& path, vector<string>& dirs, vector<string>& files)
{
	...
	do
	{
		if (file.attrib & _A_SUBDIR)//如果是一个目录
		{
			//如果是本身目录或者上级目录就不用递归了
	...
	} while (_findnext(handle, &file) == 0);//成功返回0，失败返回-1
